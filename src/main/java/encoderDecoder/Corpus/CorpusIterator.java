...
public class CorpusIterator implements MultiDataSetIterator {
    ...
    /*
     * Motivation: I want to get asynchronous data iteration while not blocking on network.fit() until the end of epoch. I want to checkpoint
    ...
    private int dictonarySize;
    private int rowSize;
    private MultiDataSetPreProcessor preProcessor;

    public CorpusIterator(List<List<Double>> corpus, List<List<Double>> label, int batchSize, int batchesPerMacrobatch, int dictonarySize, int rowSize) {
        this.corpus = corpus;
        this.label = label;
        this.batchSize = batchSize;
        this.batchesPerMacrobatch = batchesPerMacrobatch;
        this.dictonarySize = dictonarySize;
    ...
    public MultiDataSet next(int num) {
        ...
        INDArray prediction = Nd4j.zeros(currentBatchSize, dictonarySize, rowSize);
        INDArray decode = Nd4j.zeros(currentBatchSize, dictonarySize, rowSize);
        ...
        for (int j = 0; j < currentBatchSize; j++) {
            ...
            List<Double> rowPred = new ArrayList<>(label.get(i));
            ...
            double predOneHot[][] = new double[dictonarySize][rowPred.size()];
            double decodeOneHot[][] = new double[dictonarySize][rowPred.size()];
            ...
            prediction.put(new INDArrayIndex[] { NDArrayIndex.point(j), NDArrayIndex.interval(0, dictonarySize),
                    NDArrayIndex.interval(0, rowPred.size()) }, Nd4j.create(predOneHot));
            decode.put(new INDArrayIndex[] { NDArrayIndex.point(j), NDArrayIndex.interval(0, dictonarySize),
